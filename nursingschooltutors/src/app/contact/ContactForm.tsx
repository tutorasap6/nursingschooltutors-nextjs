...
export default function ContactForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch('/api/contact', {
        ...
        body: JSON.stringify({ ...data, budget_selected: selectedChip, budget_custom: budget }),
      })
      if (res.ok) { setStatus('sent'); form.reset(); setService(''); setBudget(''); setSelectedChip(''); }
  ...
  return (
    <div className="contact-form-wrap">
      ...
      {status === 'sent' && (
        <div className="form-success">✅ Message received! We&apos;ll contact you within 30 minutes via WhatsApp or email.</div>
      )}
      {status === 'error' && (
        <div className="form-error">❌ Something went wrong. Please WhatsApp us directly at +1(765)470-9090.</div>
      )}
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          ...
          <textarea id="message" name="message" required placeholder="e.g. I need help with NR-507 Advanced Pathophysiology at Chamberlain. 8-week course, weekly discussions, midterm and final. Deadline is in 5 days..."/>
        </div>
      ...
      <a href="https://wa.me/17654709090?text=Hi%20NursingSchoolTutors!%20I%20need%20help%20with%20my%20nursing%20class%20or%20exam." target="_blank" rel="noopener"
         className="btn btn-whatsapp btn-lg" style={{width:'100%',justifyContent:'center',marginTop:'12px'}}>
      ...
      <style>{`
        ...
        .budget-header label { display: block; font-weight: 700; font-size: 14px; color: var(--text); margin-bottom: 2px; }
        .budget-hint { font-size: 12.5px; color: var(--text-light); }
        ...
        .chip-range { display: block; font-size: 15px; font-weight: 800; color: var(--jungle-dark); }
        ...
        .dollar-sign { background: var(--jungle-green); color: white; padding: 13px 14px; font-weight: 700; font-size: 16px; flex-shrink: 0; }
